...
// crc 1ms 한번씩 56번
char DECHEX[9];
unsigned char CRC[2]="00";
unsigned char CRC_A[4];

void CHK_CRC16( unsigned char dataa )
{
	static unsigned char TABLE_L[256]= {
	...
	static unsigned char TABLE_H[256]={
	...
	unsigned char CRC_BL,CRC_BH;
	 ...
	 CRC_BL=0;
	 for(k=0;k<8;k++){       // xor conversion
		if(((dataa>>k)&0x01)!=((CRC[1]>>k)&0x01)){
		  CRC_BL+=0x01<<k;
		}
	 }
	 CRC_BH=TABLE_L[CRC_BL];   CRC[1]=0;
	 for(k=0;k<8;k++){       // xor conversion
		if(((CRC_BH>>k)&0x01)!=((CRC[0]>>k)&0x01)){
		  CRC[1]+=0x01<<k;
		}
	 }
	 CRC[0]=TABLE_H[CRC_BL];
...
void himix_drive(void)
{
	...
	HIMIX.timer = 0;
	if(HIMIX.index++ > 8)		HIMIX.index = 0;		// 대략 1초 이상 주기로 패킷을 한 개씩 3종류
	...
	if(HIMIX.index == 0)
	{
		...
		for(i = 3; i <= 56; i++)	//Test Packet
			HIMIX.tx_buffer[i] = i;

		CRC[0]=CRC[1]=0;
		//for(k=1;k<56;k++)  CHK_CRC16((unsigned char)HIMIX.tx_buffer[k]);
		for(k=0;k<2;k++)
		{
			if((CRC[k]/16) <= 9)
				CRC_A[k*2] = CRC[k]/16+'0';
			else
				CRC_A[k*2] = CRC[k]/16-10+'a';
				
			if((CRC[k]%16) <= 9)
				CRC_A[k*2+1] = CRC[k]%16+'0';
			else
				CRC_A[k*2+1] = CRC[k]%16-10+'a';
		}

		HIMIX.tx_buffer[57] = CRC_A[0];
		HIMIX.tx_buffer[58] = CRC_A[1];
		HIMIX.tx_buffer[59] = CRC_A[2];
		HIMIX.tx_buffer[60] = CRC_A[3];

		//패킷 보내기 위한 명령 수행			
		HIMIX_SEND;			//485 RTS On (Send Mode)
		*ScicRegs_SCICTL2 |= 0x0001;// tx interrupt 활성						
		HIMIX.tx_count = 1;// tx intrrupt 활성화 후 최초 한번 써야함
		*ScicRegs_SCITXBUF = HIMIX.tx_buffer[0];
	} else if(HIMIX.index == 4) {
		HIMIX.tx_buffer[0] = 0x7e;
		HIMIX.tx_buffer[1] = 0x30;
		...
		*ScicRegs_SCITXBUF = HIMIX.tx_buffer[0];
		...
		HIMIX.tx_buffer[57] = 0xA1;//CRC_A[0];
		HIMIX.tx_buffer[58] = 0xA2;//CRC_A[1];
		HIMIX.tx_buffer[59] = 0xA3;//CRC_A[2];
		HIMIX.tx_buffer[60] = 0xA4;//CRC_A[3];
